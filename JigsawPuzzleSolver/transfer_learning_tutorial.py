...
for body in body_models:
    if type(body) is torch.nn.modules.conv.Conv2d:
        tensor = body.weight.data.cpu().numpy()
        #tensor = np.swapaxes(tensor, 1, 2)
        #tensor = np.swapaxes(tensor, 2, 3)
        plot_kernels(tensor, 8)
    elif type(body) is nn.modules.container.Sequential:
        for block in body:
            tensor = block.conv1.weight.data.cpu().numpy()
            for i in range(len(tensor)):
                plot_kernels(tensor[i][i].reshape((1, 3, 3)), 3)
            tensor = block.conv2.weight.data.cpu().numpy()
            for i in range(len(tensor)):
                plot_kernels(tensor[i][i].reshape((1, 3, 3)), 3)
'''
plt.show()
